...
public class LazyInitLate2Arg<X,Y,T> {//implements BiFunction<X, Y, T> {
	T builtObject;
	...
	public LazyInitLate2Arg(BiFunction<X,Y,T> builder) {
		super();
		this.builder = builder;
	}
...
//	@Override
	public T get_initWithArgAtFirstCall(X x, Y y) {
		if(builtObject == null)
			builtObject = builder.apply(x,y);
		return builtObject;
	}
	public static <X,Y,T> LazyInitLate2Arg<X,Y,T> of(BiFunction<X,Y,T> builder){
		return new LazyInitLate2Arg<X,Y,T>(builder);
	}
